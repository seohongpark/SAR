...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, vf_latent_vector, init_scale=1.0, init_bias=0.0, log_std_min=-20):
        mean = linear(pi_latent_vector, 'pi', self.size, init_scale=init_scale, init_bias=init_bias)
        logstd_param = tf.get_variable(name='pi/logstd_param', shape=[1, self.size - 1], initializer=tf.zeros_initializer())
        logstd_linear = linear(pi_latent_vector, 'pi/logstd_init', 1)
        logstd = tf.clip_by_value(tf.concat([mean[:, :-1] * 0.0 + logstd_param, logstd_linear], axis=1), log_std_min, 2)
