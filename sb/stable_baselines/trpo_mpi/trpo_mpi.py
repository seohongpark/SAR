        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    for k in range(self.g_step):
                        ...
                        if np.allclose(grad, 0):
                            ...
                            for (loss_name, loss_val) in zip(self.loss_names, mean_losses):
                                logger.record_tabular(f'train/{loss_name}', loss_val)
                    ...
                    logger.record_tabular("train/explained_variance_tdlam_before",
                    ...
                    if len(len_buffer) > 0:
                        logger.record_tabular("rollout/ep_len_mean", np.mean(len_buffer))
                        logger.record_tabular("rollout/ep_reward_mean", np.mean(reward_buffer))
                    if self.using_gail:
                        logger.record_tabular("train/EpTrueRewMean", np.mean(true_reward_buffer))
                    logger.record_tabular("train/EpThisIter", len(lens))
                    ...
                    logger.record_tabular("train/EpisodesSoFar", episodes_so_far)
                    logger.record_tabular("time/total_timesteps", self.num_timesteps)
                    logger.record_tabular("train/TimeElapsed", time.time() - t_start)
                    for key, value in seg['infos'].items():
                        logger.logkv(f'rollout/{key}_mean', np.mean(value))

                    if self.verbose >= 1 and self.rank == 0:
                        logger.dump_tabular(step=self.num_timesteps)
