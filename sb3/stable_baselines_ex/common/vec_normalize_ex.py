...
class VecNormalizeEx(VecEnvWrapper):
    """
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        if self.training:
            self._update_reward(rewards, np.array([info['w'] for info in infos]))
    ...
    def _update_reward(self, reward: np.ndarray, w=None) -> None:
        """Update reward normalization statistics."""
        if w is None:
            self.ret = self.ret * self.gamma + reward
        else:
            self.ret = self.ret * (self.gamma ** w) + reward
